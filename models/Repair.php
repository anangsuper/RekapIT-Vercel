...
class Repair {
    ...
    public function countAll($id_cabang = null, $search = null) {
        $query = "SELECT COUNT(r.id) FROM " . $this->table . " r
                  JOIN assets a ON r.asset_id = a.id
                  WHERE 1=1";
        if ($id_cabang) $query .= " AND a.id_cabang = :id_cabang";
        if ($search) {
            $query .= " AND (a.kode_aset LIKE :search OR a.nama_aset LIKE :search OR r.keluhan LIKE :search OR r.tindakan LIKE :search OR r.status LIKE :search)";
        }
        $stmt = $this->conn->prepare($query);
