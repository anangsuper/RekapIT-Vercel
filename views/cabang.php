...
// Fetch branches with asset counts and total repair costs
$stmt = $conn->prepare("SELECT c.*, 
                               (SELECT COUNT(a.id) FROM assets a WHERE a.id_cabang = c.id) as total_aset,
                               COALESCE((SELECT SUM(r.biaya) FROM repairs r JOIN assets a2 ON r.asset_id = a2.id WHERE a2.id_cabang = c.id), 0) as total_biaya
                        FROM cabang c 
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm rounded-4 position-relative overflow-hidden" style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%);">
                <div class="card-body p-4 text-white position-relative">
                    ...
                    <span class="small fw-bold opacity-75">TOTAL BIAYA REPAIR</span>
                    <h3 class="fw-800 mb-0 mt-1">Rp <?= number_format($totalBiayaAll, 0, ',', '.') ?></h3>
                    <small class="opacity-70 d-block mt-2">Total pengeluaran perbaikan seluruh cabang</small>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            ...
                            <tr class="branch-row" data-search="<?= htmlspecialchars(strtolower($c['nama_cabang'] . ' ' . $c['alamat'])) ?>">
                                ...
                                <td>
                                    <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-3 py-2 fw-bold" style="font-size: 0.72rem;">
                                        💰 Rp <?= number_format($c['total_biaya'], 0, ',', '.') ?>
                                    </span>
                                </td>
                                <td>
                    ...
                    <div class="card border p-3 mb-3 rounded-3 shadow-sm mobile-branch-card" data-search="<?= htmlspecialchars(strtolower($c['nama_cabang'] . ' ' . $c['alamat'])) ?>">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <div class="d-flex gap-2">
                                ...
                                <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-2.5 py-1.5 fw-bold" style="font-size: 0.7rem;">
                                    💰 Rp <?= number_format($c['total_biaya'], 0, ',', '.') ?>
                                </span>
                            </div>
                        </div>
