...
// Fetch Branch Name
$selected_cabang = (int)$selected_cabang;
$stmt = $conn->prepare("SELECT nama_cabang FROM cabang WHERE id = :id");
$stmt->bindValue(':id', $selected_cabang, PDO::PARAM_INT);
$stmt->execute();
...
<table border="1">
    ...
    <tr>
        ...
        <?php
        $displayTindakan = $dm['tindakan'];
        if (strpos($displayTindakan, 'Checklist: ') !== false) {
            $parts = explode('. Tindakan: ', $displayTindakan);
            $chkText = $parts[0];
            $actText = isset($parts[1]) ? $parts[1] : 'Pengecekan Rutin';
            $displayTindakan = $actText . " (" . $chkText . ")";
        } else {
            $displayTindakan = $displayTindakan ?: 'Pengecekan Rutin';
        }
        ?>
        <td><?= htmlspecialchars($displayTindakan) ?></td>
